...
__all__ = ['mb_item', 'LossCallback', 'hook_out', 'KL', 'SymmetrizedKL', 'adv_project', 'compute_adversarial_loss',
           'ALUMCallback', 'update_ema_model', 'SMARTCallback', 'VATCallback']
...
# Cell
def mb_item(x):
    return x.item() if (isinstance(x, Tensor) and hasattr(x, 'item')) else x
...
class LossCallback(Callback):
    ...
    def log_loss(self, loss:torch.Tensor, log_name:str):
        ...
        log_extras[log_name] = mb_item(loss)
...
class ALUMCallback(LossCallback):
    ...
    def after_loss(self):
        if self.epoch >= self.start_epoch and self._do_vat:
            ...
            self.log_loss(0, 'adversarial_loss')
